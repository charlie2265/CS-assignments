...
// The WriteFile function populates a struct array of size 3 and 
// writes the array to a binary file, 
// then closes the file and returns Client struct.
Client WriteFile(){
...
// GetID function takes Client array as argument 
// prompts user for ID and searches (linear seacrh) 
// array for ID. and returns Client struct.
Client GetID(Client* arr){
    char id[15];
    int idx = 0;
    ...
    while(idx < 3 && strncmp(id, arr[idx].id, 15) != 0 )
        ++idx;
    if (idx < 3)
        arr[idx].outP(cout);
    else
...
// the file, because either it existed or it was just created. Initialize now opens the file and reads in the customer array and 
// returns.Define a struct to store a customer record in a file (ID, company name, city, state, customer type)

// Populate an array of clients with at least three clients' information (can be hard-coded or retrieved from a file, your choice)
...
// You are encouraged to store the information on disk, either in a sequential or random-access file, but that is optional this week